...
class GameBloc extends Bloc<GameEvent, GameState> {
  ...
  Future<void> _onLetterGuessed(GameLetterGuessed event, Emitter<GameState> emit) async {
    final current = state;
    if (current is! GameRunning) return;

    final respostaNorm = Validators.normalizarResposta(current.pergunta.resposta);
    ...
    if (result.partida.estado == domain.EstadoPartida.gameOver) {
      final goResult = await gameOver.executar(result.partida, result.progresso);
      emit(GameOver(
      ...
      if (palavraCompleta) {
        try {
          ...
          LoggingService.error('Erro ao carregar próxima pergunta', erro: e, stackTrace: st);
          emit(GameError(e.toString()));
        }
      } else {
        emit(GameRunning(
  ...
  Future<void> _onContinued(GameContinued event, Emitter<GameState> emit) async {
    final current = state;
  ...
  Future<void> _onRestarted(GameRestarted event, Emitter<GameState> emit) async {
    final current = state;
    if (current is! GameOver) return;
